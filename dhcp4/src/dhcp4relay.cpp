...
DHCPCounterTable dhcpCntrTable;
...
int prepare_vlan_sockets (relay_config &config) {
    ...
    if (setsockopt(client_sock, SOL_SOCKET, SO_BROADCAST, &broadcastEnable, sizeof(broadcastEnable)) < 0) {
    ...
    if (setsockopt(client_sock, IPPROTO_IP, IP_PKTINFO, &optval, sizeof(optval)) < 0) {
        syslog(LOG_ERR, "[DHCPV4_RELAY] setsockopt: Failed to set socket option to "
...
void encode_relay_option(pcpp::DhcpLayer *dhcp_pkt, relay_config *config)
{
    ...
    if ((dhcp_pkt->getHeaderLen() + buf_offset) > MAX_DHCP_PKT_SIZE) {
        syslog(LOG_ERR, "[DHCPV4_RELAY] %ld packet size is exceeding allowed size %d"
                         " from interface %s", (dhcp_pkt->getHeaderLen() + buf_offset),
...
void from_client(pcpp::DhcpLayer *dhcp_pkt, relay_config &config)
{
    ...
    if (!(dhcp_pkt->getDhcpHeader()->gatewayIpAddress)) {
        ...
        if (config.agent_relay_mode == "forward_and_replace") {
            ...
            syslog(LOG_INFO, "[DHCPV4_RELAY] agent relay mode is forward untouch,"
                             " not appending self relay info %s", config.vlan.c_str());
        } else {
    ...
    for(auto server : config.servers_sock) {
        if(send_udp(sock, (uint8_t *)dhcp_pkt->getDhcpHeader(), server, dhcp_pkt->getHeaderLen())) {
            ...
            dhcpCntrTable.incrementCounter(config.vlan, "TX", (int)dhcp_pkt->getMessageType());
        } else {
            syslog(LOG_NOTICE, "[DHCPV4_RELAY] DHCP packet sending FAILED for configured server: %s, interface: %s",
                             config.servers[index].c_str(), config.vlan.c_str() );
            // increment drop counter
            dhcpCntrTable.incrementCounter(config.vlan, "TX", DHCPv4_MESSAGE_TYPE_DROP);
        }
        index++;
    }
}
                ...
                std::string src_ip)
{
    ...
    if (circuit_id_ptr == NULL) {
        syslog(LOG_ERR, "[DHCPV4_RELAY] Circuit id sub-option is missing in relay"
               " agent option from server %s", src_ip.c_str());
        return;
    }
    ...
    if (vlan_interface.length() > 0) {
        ...
        if (config_itr == vlans->end()) {
            syslog(LOG_INFO, "[DHCPV4_RELAY] Vlan config not found for the circuit"
    ...
    dhcpCntrTable.incrementCounter(config.vlan, "RX", (int)dhcp_pkt->getMessageType());
